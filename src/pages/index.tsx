...
export default function Home() {
  ...
  // const [sortOption, setSortOption] = useState("NUMBER_LOW_TO_HIGH");
...
//     if (event.target.checked) {
//       // If the checkbox is checked, add it to the selectedCheckboxes array
//       // setSelectedTypes((prevSelected) => [...prevSelected, checkboxValue]);
//       setSelectedTypes([...selectedTypes, checkboxValue]);
//     } else {
//       // If the checkbox is unchecked, remove it from the selectedCheckboxes array
//       setSelectedTypes((prevSelected) =>
...
//     switch (event.target.value) {
//       case "NUMBER_LOW_TO_HIGH":
//         setResults(results.slice().sort((a: { details: { id: number; }; }, b: { details: { id: number; }; }) => a.details.id - b.details.id));
//         break;

//       case "NUMBER_HIGH_TO_LOW":
//         setResults(results.slice().sort((a: { details: { id: number; }; }, b: { details: { id: number; }; }) => b.details.id - a.details.id));
//         break;

//       case "ALPHA_A_TO_Z":
//         setResults(results.slice().sort((a: { name: number; }, b: { name: number; }) => {
//           if (a.name < b.name) {
//             return -1;
//           }
//           if (a.name > b.name) {
...
//                     {pokemonTypeNames.map((type) => (
//                       <div className="mx-2 my-2 flex justify-between">
//                         <input className="mx-3" type="checkbox" name="type-filter" value={type} onChange={handleFilter} disabled={isCheckboxDisabled(type)}/>
//                         <PokemonType type={type} />
//                       </div>
//                     ))}
//                   </div>
